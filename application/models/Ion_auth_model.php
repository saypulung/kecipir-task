...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id,first_name,last_name, password, active, last_login')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
	...
	public function groups_json(){
        ...
        $this->datatables->add_column('view', '<a href="'.site_url('auth/edit_group/').'$1" class="btn btn-primary btn-xs">edit</a>  <a href="'.site_url('auth/delete_group/').'$1" class="btn btn-danger btn-xs">delete</a>', 'id');
        return $this->datatables->generate();
	}
	public function users_json(){
		$this->datatables->select('id,ip_address,username,email,last_login,active,first_name,last_name,company,phone,groups');
        $this->datatables->from('v_users');
        $this->datatables->add_column('view', '<a href="'.site_url('auth/edit_user/').'$1" class="btn btn-primary btn-xs">edit</a> <a href="'.site_url('auth/delete_user/').'$1"  class="btn btn-danger btn-xs">delete</a>', 'id');
        return $this->datatables->generate();	
	}
